...
local NS430CommandGenerator = require("SharkPlanner.Modules.NS-430.NS430CommandGenerator")
...
COMMAND_GENERATORS["NS-430"] = NS430CommandGenerator
...
local function getConfigurationDefinition()
    return {
        SectionName = "NS-430", -- defines top most section name,        
        {
            SectionName = "NS-430", -- subsection should be named according to the name of the device. Each used device should have own subsection
            Options = {
                {
                    Name = "CommandDelay", -- defines the name of option. Later you can reference this particular option as 'MyModule.MyEntryDevice.CommandDelay'
...
local function determineVariant(base_module_name)
    if base_module_name == "NS-430" then
